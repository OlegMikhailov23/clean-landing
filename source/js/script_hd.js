...
$(function () {
    $('#introYborkaForm').submit(function (e) {
        e.preventDefault(); //это чтобы форма не отправлялась через браузер, только по AJAX
        var formdata = 'room-count=' + $('#room-count').val() + '&bath-count=' + $('#bath-count').val() + '&intro-form-tel=' + $('#intro-form-tel').val();
        //запишем все данные формы в переменную data
        $.ajax({
            url: 'form-intro.php',
            data: formdata,
            type: 'post',
            success: function (respond) {
                $(".temno").css('display', 'block') && $(".messages").css('display', 'block') && $("#room-counte").val("") && $("#bath-count").val("");
            },
            error: function () {
                alert('Произошла ошибка. Повторите попытку позже.');
            }
